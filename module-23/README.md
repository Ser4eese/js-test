...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && req.url === '/api/status') {
    ...
    // Для POST нужно читать body (см. следующий модуль)
...
A: Номер (1-65535), по которому программа принимает соединения. Веб-серверы обычно используют 80 (HTTP) или 443 (HTTPS). Для разработки — 3000, 8080.

**Q: Почему Content-Type важен?**

A: Браузер/клиент должен знать, как интерпретировать ответ. `application/json` говорит — это JSON, парси как объект.

**Q: Чем это отличается от Express?**

A: Express — фреймворк поверх http-модуля. Добавляет удобный роутинг, middleware, обработку body. Начинаем с чистого http, чтобы понять основы.

**Q: Как остановить сервер?**

A: `Ctrl+C` в терминале, или `server.close()` программно.
